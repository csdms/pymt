...
def test_raster_field():
    ...
    assert_array_equal(
        g.get_field("var0").data, np.array([0., 1., 2., 3., 4., 5.], dtype=np.float64)
    )
...
def test_mapper():
    ...
    assert_array_equal(src.get_x(), np.array([0., 1., 2., 0., 1., 2., 0., 1., 2.]))
    assert_array_equal(src.get_y(), np.array([0., 0., 0., 1., 1., 1., 2., 2., 2.]))
