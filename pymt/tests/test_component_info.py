...
class TestConfigFile(unittest.TestCase):
    ...
    def make_temp_file(self, infos):
        (handle, name) = tempfile.mkstemp(dir=os.getcwd(), suffix='.cfg',
                                          text=True)
    ...
    def make_empty_temp_file(self):
        (_, name) = tempfile.mkstemp(dir=os.getcwd(), suffix='.cfg',
                                          text=True)
        self._temp_files.append(name)
...
class TestToConfigFile(TestConfigFile):
    def test_empty_dict(self):
        ...
        self.assertSetEqual(_VALID_KEYS,
                            set(config.options('section.name')))
    ...
    def test_ignore_extra_params(self):
        name = self.make_empty_temp_file()
        to_config_file(name, 'section.name', {'invalid_parameter': 'empty',
                                              'port_queue_dt': 1.2})
    ...
    def test_dict_with_two_entries(self):
        name = self.make_empty_temp_file()
        to_config_file(name, 'section.name',
                       {'template_files': {'infile1': 'outfile1',
                                           'infile2': 'outfile2'}
                       })
    ...
    def test_list_with_two_entries(self):
        ...
        config = ConfigParser()
        config.read(name)
        self.assertListEqual(['port1', 'port2'],
                             config.get('section.name', 'ports').split(','))
...
class TestNamesWithPrefix(unittest.TestCase):
    def test_empty_prefix(self):
        names = ['name1', 'name2']
        self.assertSetEqual(set(['name1', 'name2']),
                            names_with_prefix(names, ''))
    ...
    def test_none_prefix(self):
        names = ['name1', 'name2']
        self.assertSetEqual(set(['name1', 'name2']),
                            names_with_prefix(names, None))

    def test_with_base(self):
        names = ['base.name1', 'base.name2']
        self.assertSetEqual(set(['base.name1', 'base.name2']),
                            names_with_prefix(names, 'base'))
    ...
    def test_different_bases(self):
        ...
        self.assertSetEqual(set(['base1.name1']),
                            names_with_prefix(names, 'base1'))
        self.assertSetEqual(set(['base2.name2']),
                            names_with_prefix(names, 'base2'))
