...
def is_rectilinear(grid, strict=True):
    loosely_rectilinear = (is_callable_method(grid, 'get_shape') and
                            is_callable_method(grid, 'get_xyz_coordinates'))
    if not strict:
...
def is_structured(grid, strict=True):
    ...
    if not strict:
        ...
        return loosely_structured and not (is_rectilinear(grid) or
                                           is_uniform_rectilinear(grid))
...
def is_unstructured(grid, strict=True):
    if not strict:
        ...
        return not (is_structured(grid) or is_rectilinear(grid) or
                    is_uniform_rectilinear(grid))
