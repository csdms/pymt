...
class CellToPoint (IGridMapper):
    ...
    def run (self, src_values, dst_vals=None, bad_val=-999.):
        ...
        rtn = np.choose (src_values.take (self._map)<bad_val, (src_values.take (self._map), dst_vals))
    ...
    def test (self, dst_grid, src_grid):
        ...
        #return (all (np.diff (src_grid.get_offset ())>2) and
        #        all (np.diff (dst_grid.get_offset ())==1))
    def name (self):
        return 'CellToPoint'
...
if __name__ == "__main__":
    ...
    #import cProfile
    #cProfile.run ('run_test (80, 160)', 'mapper.prof')
