...
from cmt.grids import (RasterField, RectilinearField, StructuredField,
                       UnstructuredField)
from cmt.vtk import tofile as field_tofile

class TestVtk (unittest.TestCase):
    def setUp (self):
        for file in self.vtk_files.values ():
            try:
                os.remove (file)
            except OSError:
                pass
...
class TestUniformRectilinear (TestVtk):
    ...
    def test_1d (self):
        ...
        field = RasterField ((12, ), (1, ), (-1, ), indexing='ij',
                             units=('m', ))
        data = np.arange (12.)
        field.add_field ('Elevation', data, centering='point')
...
class TestStructured (TestVtk):
    ...
    def test_1d (self):
        ...
        field = StructuredField ((1, 2, 4, 5), (4, ), indexing='ij',
                                 units=('m', ))
        data = np.arange (4.)
        field.add_field ('Elevation', data, centering='point')
...
class TestUnstructured (TestVtk):
    ...
    def test_1d (self):
        ...
        field.add_field ('point_field', np.arange (4),
                         centering='point', units='C')
        field.add_field ('cell_field', np.arange (3)*10.,
                         centering='zonal', units='F')
