...
def test_timeinterp():
    interp = TimeInterpolator(((0.0, 1.0), (1.0, 2.0), (2.0, 3.0)))
...
def test_timeinterp_multiple_interps():
    interp = TimeInterpolator()
    interp.add_data(((0.0, 1.0), (1.0, 2.0)))
    assert interp(0.5) == approx(1.5)

    interp.add_data(((2.0, 3.0),))
    assert interp(0.5) == approx(1.5)
...
def test_timeinterp_interpolate_method():
    interp = TimeInterpolator(((0.0, 1.0), (1.0, 2.0), (2.0, 3.0)))
    assert interp.interpolate(0.5) == approx(1.5)
...
def test_timeinterp_with_arrays():
    ...
    interp = TimeInterpolator(((0.0, arr * 1), (1.0, arr * 2), (2.0, arr * 3)))

    assert interp(0.5) == approx(np.full(shape, 1.5))
...
def test_interp_with_fill_value(method):
    interp = TimeInterpolator(
        ((0.0, 1.0), (1.0, 2.0), (2.0, 3.0), (3.0, 4.0)), method=method, fill_value=-1.0
    )
